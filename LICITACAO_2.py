"""
RADAR NSC — Monitor de Licitações de Medicamentos Estratégicos no PNCP (v1)
=============================================================================
Busca licitações publicadas no Portal Nacional de Contratações Públicas
que contenham termos de medicamentos estratégicos (oncológicos, imunológicos,
neurológicos e outros de alto custo) no objeto da compra.

Mesma arquitetura do Radar Alto Custo / FarmaUSA Cannabis:
- API Search do PNCP (busca textual por data de divulgação)
- Filtra por keywords específicas dos 17 medicamentos monitorados
- Salva no Supabase (tabela editais_nsc / itens_nsc)
...
log = logging.getLogger("radar_nsc")

# ================= CONFIGURAÇÕES =================
TELEGRAM_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN_NSC", "")
CHAT_ID = os.environ.get("TELEGRAM_CHAT_ID_NSC", "")
SUPABASE_URL = os.environ.get("SUPABASE_URL_NSC", "")
SUPABASE_KEY = os.environ.get("SUPABASE_KEY_NSC", "")

if not TELEGRAM_BOT_TOKEN:
    TELEGRAM_BOT_TOKEN = "8399556844:AAGQhzzeozJcbsOkCMqGIT5lZdYeD1HrZ5Q"
if not CHAT_ID:
    CHAT_ID = "-5244728652"
if not SUPABASE_URL:
    SUPABASE_URL = "https://fuqdhlsigarrqbvudall.supabase.co"
if not SUPABASE_KEY:
    SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ1cWRobHNpZ2FycnFidnVkYWxsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM3OTgxMTMsImV4cCI6MjA4OTM3NDExM30.0QRON5_gficUw0gpWhTxZenveTTLKbJPMT5jq1v5Et4"
...
KEYWORDS = [
    ...
    "anti-PD-1", "anti PD-1", "anti PD1",
    # Jakafi (ruxolitinibe)
    "ruxolitinibe", "ruxolitinib", "jakafi",
    "inibidor JAK", "inibidor JAK1", "inibidor JAK2",
    # Xtandi (enzalutamida)
    "enzalutamida", "enzalutamide", "xtandi",
    # Votrient (pazopanibe)
    "pazopanibe", "pazopanib", "votrient",
    # Crysvita (burosumabe)
    ...
    "anti-CD20",
    # Vyvgart (efgartigimode)
    "efgartigimode", "vyvgart",
    "anticorpo FcRn", "anti-FcRn",
    # Skyrizi (risanquizumabe)
    "risanquizumabe", "risankizumab", "skyrizi",
    "anti-IL23", "anti-IL-23",
    # Entyvio (vedolizumabe)
    "vedolizumabe", "vedolizumab", "entyvio",
    "anti-integrina",
    # Aduhelm (aducanumabe)
    "aducanumabe", "aducanumab", "aduhelm",
    "anti-beta amiloide",

    # 🧠 EPILEPSIA / NEUROLÓGICOS
    # Briviact (brivaracetam)
    "brivaracetam", "brivaracetamo", "briviact",
    # Fampyra (fampridina)
    "fampridina", "fampridine", "fampyra",
    # Ospolot (sultiame)
    "sultiame", "sultiam", "ospolot",
    # Ontozry (cenobamato)
    "cenobamato", "cenobamate", "ontozry",

    # 💊 OUTROS ESTRATÉGICOS
    # Proglicem (diazóxido)
    "diazoxido", "diazóxido", "diazoxide", "proglicem", "proglycem",
    "hiperinsulinismo",
    # Biktarvy
...
# Keywords para busca na API Search (termos principais que retornam resultados)
KEYWORDS_BUSCA = [
    # Oncológicos
...
# Mapeamento medicamento → categoria (para identificação no alerta)
MEDICAMENTO_CATEGORIA = {
    "pembrolizumabe": ("Keytruda", "🔬 Oncológico"),
    "pembrolizumab": ("Keytruda", "🔬 Oncológico"),
    "keytruda": ("Keytruda", "🔬 Oncológico"),
    "anti-pd-1": ("Keytruda", "🔬 Oncológico"),
    "anti pd-1": ("Keytruda", "🔬 Oncológico"),
    "anti pd1": ("Keytruda", "🔬 Oncológico"),
    "ruxolitinibe": ("Jakafi", "🔬 Oncológico"),
    "ruxolitinib": ("Jakafi", "🔬 Oncológico"),
    "jakafi": ("Jakafi", "🔬 Oncológico"),
    "inibidor jak": ("Jakafi", "🔬 Oncológico"),
    "inibidor jak1": ("Jakafi", "🔬 Oncológico"),
    "inibidor jak2": ("Jakafi", "🔬 Oncológico"),
    "enzalutamida": ("Xtandi", "🔬 Oncológico"),
    "enzalutamide": ("Xtandi", "🔬 Oncológico"),
    "xtandi": ("Xtandi", "🔬 Oncológico"),
    "pazopanibe": ("Votrient", "🔬 Oncológico"),
    "pazopanib": ("Votrient", "🔬 Oncológico"),
    "votrient": ("Votrient", "🔬 Oncológico"),
    "burosumabe": ("Crysvita", "🔬 Oncológico"),
    "burosumab": ("Crysvita", "🔬 Oncológico"),
    "crysvita": ("Crysvita", "🔬 Oncológico"),
    ...
    "anti-cd20": ("Ocrevus", "🧬 Imunológico"),
    "efgartigimode": ("Vyvgart", "🧬 Imunológico"),
    "vyvgart": ("Vyvgart", "🧬 Imunológico"),
    "anticorpo fcrn": ("Vyvgart", "🧬 Imunológico"),
    "anti-fcrn": ("Vyvgart", "🧬 Imunológico"),
    "risanquizumabe": ("Skyrizi", "🧬 Imunológico"),
    "risankizumab": ("Skyrizi", "🧬 Imunológico"),
    "skyrizi": ("Skyrizi", "🧬 Imunológico"),
    "anti-il23": ("Skyrizi", "🧬 Imunológico"),
    "anti-il-23": ("Skyrizi", "🧬 Imunológico"),
    "vedolizumabe": ("Entyvio", "🧬 Imunológico"),
    "vedolizumab": ("Entyvio", "🧬 Imunológico"),
    "entyvio": ("Entyvio", "🧬 Imunológico"),
    "anti-integrina": ("Entyvio", "🧬 Imunológico"),
    "aducanumabe": ("Aduhelm", "🧠 Neurológico"),
    "aducanumab": ("Aduhelm", "🧠 Neurológico"),
    "aduhelm": ("Aduhelm", "🧠 Neurológico"),
    "anti-beta amiloide": ("Aduhelm", "🧠 Neurológico"),
    "brivaracetam": ("Briviact", "🧠 Neurológico"),
    "brivaracetamo": ("Briviact", "🧠 Neurológico"),
    "briviact": ("Briviact", "🧠 Neurológico"),
    "fampridina": ("Fampyra", "🧠 Neurológico"),
    "fampridine": ("Fampyra", "🧠 Neurológico"),
    "fampyra": ("Fampyra", "🧠 Neurológico"),
    "sultiame": ("Ospolot", "🧠 Neurológico"),
    "sultiam": ("Ospolot", "🧠 Neurológico"),
    "ospolot": ("Ospolot", "🧠 Neurológico"),
    "cenobamato": ("Ontozry", "🧠 Neurológico"),
    "cenobamate": ("Ontozry", "🧠 Neurológico"),
    "ontozry": ("Ontozry", "🧠 Neurológico"),
    "diazoxido": ("Proglicem", "💊 Estratégico"),
    "diazóxido": ("Proglicem", "💊 Estratégico"),
    "diazoxide": ("Proglicem", "💊 Estratégico"),
    "proglicem": ("Proglicem", "💊 Estratégico"),
    "proglycem": ("Proglicem", "💊 Estratégico"),
    "hiperinsulinismo": ("Proglicem", "💊 Estratégico"),
    "biktarvy": ("Biktarvy", "💊 Estratégico"),
    "bictegravir": ("Biktarvy", "💊 Estratégico"),
    "tenofovir alafenamida": ("Biktarvy", "💊 Estratégico"),
    "emtricitabina": ("Biktarvy", "💊 Estratégico"),
}
...
DASHBOARD_URL = "https://igdata.com.br"
...
def identificar_medicamento(texto):
    """Identifica qual medicamento foi encontrado no texto e retorna (nome_comercial, categoria)"""
    if not texto:
        return None, None
    texto_lower = str(texto).lower()
    for kw, (nome, cat) in MEDICAMENTO_CATEGORIA.items():
        if kw.lower() in texto_lower:
            return nome, cat
    return None, None
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for item in lista:
                ...
                med_nome, med_cat = identificar_medicamento(desc)
...
def enviar_telegram(edital, itens_texto, modalidade_label):
    ...
    # Identificar medicamentos encontrados
    medicamentos_encontrados = set()
    categoria_principal = None
    for it in itens_texto:
        if it.get("medicamento"):
            medicamentos_encontrados.add(it["medicamento"])
            if not categoria_principal and it.get("categoria"):
                categoria_principal = it["categoria"]

    # Se não achou nos itens, tenta no objeto
    if not medicamentos_encontrados:
        med_nome, med_cat = identificar_medicamento(edital.get("objeto", ""))
        if med_nome:
            medicamentos_encontrados.add(med_nome)
            categoria_principal = med_cat

    tag_medicamento = ""
    if medicamentos_encontrados:
        nomes = " | ".join(sorted(medicamentos_encontrados))
        tag_medicamento = f"\n💊 <b>{nomes}</b>"
        if categoria_principal:
            tag_medicamento += f"  ({categoria_principal})"
    ...
    msg = (
        f"🏥 <b>NOVA LICITAÇÃO — MEDICAMENTO ESTRATÉGICO</b>\n"
        f"{linha_local}"
        f"{tag_medicamento}\n\n"
        f"🏢 {orgao}\n"
        f"📋 {titulo}"
        ...
        f"📊 <a href=\"{DASHBOARD_URL}\">igdata.com.br</a>\n\n"
        f"<i>Alerta automático — Radar NSC</i>"
    )
...
def enviar_resumo_dia():
    ...
    if not editais:
        msg = (
            ...
            f"Nenhuma licitação de medicamento estratégico encontrada hoje.\n\n"
            f"<i>Radar NSC — igdata.com.br</i>"
        )
        try:
            requests.post(
                f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
    ...
    msg = (
        f"📊 <b>Resumo do dia — {hoje_fmt}</b>\n"
        f"🏥 {len(editais)} licitação(ões) de medicamento(s) estratégico(s)\n"
        f"\n━━━━━━━━━━━━━━━━━━━━━━\n"
        f"{corpo}"
        f"\n━━━━━━━━━━━━━━━━━━━━━━"
        f"{valor_fmt}\n"
...
def main():
    ...
    log.info("=" * 60)
    log.info(f"🏥 RADAR NSC v1 — Medicamentos Estratégicos")
    log.info(f"⏰ Execução: {agora.strftime('%d/%m/%Y %H:%M:%S')}")
    log.info("=" * 60)

    if not SUPABASE_URL or not SUPABASE_KEY:
        log.error("❌ SUPABASE_URL_NSC ou SUPABASE_KEY_NSC não configurados!")
        sys.exit(1)
    ...
    log.info(f"   Com med. estratégico:   {total_encontradas}")
    log.info(f"   Novas (Telegram):       {total_novas}")
    log.info(f"   Já conhecidas:          {total_encontradas - total_novas}")
    log.info("=" * 60)

    # Resumo diário — envia apenas na execução das 18h+ Brasília (21h+ UTC)
    if TELEGRAM_BOT_TOKEN and CHAT_ID and not APENAS_POPULAR_BANCO:
        if datetime.datetime.utcnow().hour >= 21:
            enviar_resumo_dia()

    # Mensagem de varredura concluída
    if TELEGRAM_BOT_TOKEN and CHAT_ID and not APENAS_POPULAR_BANCO:
        agora_fmt = datetime.datetime.now().strftime("%d/%m/%Y %H:%M")
        if total_novas > 0:
            msg_fechamento = (
                f"✅ <b>Varredura concluída</b> — {agora_fmt}\n\n"
                f"📊 Analisadas: {total_analisadas:,}\n"
                f"🏥 Com medicamento estratégico: {total_encontradas}\n"
                f"🆕 Novas enviadas: <b>{total_novas}</b>\n\n"
                f"<i>Radar NSC — igdata.com.br</i>"
            )
        else:
            msg_fechamento = (
                f"🔍 <b>Varredura concluída</b> — {agora_fmt}\n\n"
                f"Nenhuma licitação nova encontrada nesta execução.\n"
                f"📊 {total_analisadas:,} editais analisados.\n\n"
                f"<i>Radar NSC — igdata.com.br</i>"
            )
        try:
            requests.post(
                f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
                json={"chat_id": CHAT_ID, "text": msg_fechamento, "parse_mode": "HTML"},
                timeout=10,
            )
        except Exception as e:
            log.warning(f"Erro ao enviar fechamento: {e}")
